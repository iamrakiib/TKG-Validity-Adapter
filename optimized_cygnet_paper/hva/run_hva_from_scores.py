...
def main():
    p = argparse.ArgumentParser(description="Protocol-safe HVA exact-only runner from backbone score dumps")
    ...
    result = {
        ...
        "train_log": train_log,
        "evaluation_protocol": {
            "topk_candidate_selection": "torch.topk(base_scores.detach()) only; target labels are not used for candidate membership",
            "feature_construction": "target/object column is masked before feature construction",
