...
set -e
SEED=${1:-42}
CKPT=${2:-../checkpoints/tirgn/ICEWS18/seed${SEED}/best.pt}
cd src
python main.py -d ICEWS18 --test --eval-mode dump_valid --dump-full-scores \
  --full-score-path ../score_dumps/tirgn/ICEWS18/valid_scores_seed${SEED}.npz \
  --resume-ckpt ${CKPT} \
