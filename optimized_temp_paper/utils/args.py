...
def get_args():
    ...
    # ------------------------------------------------------------
    # HVA paper reproducibility: optional score dumping from TeMP.
    # These arguments do not change baseline training. They only allow
    # exporting candidate scores so HVA exact-only can be trained/applied
    # as the paper's ranking-stage adapter.
    # ------------------------------------------------------------
    parser.add_argument("--dump-score-dir", type=str, default=None,
                        help="Directory for TeMP candidate score chunks used by HVA.")
    parser.add_argument("--dump-score-object-only", dest="dump_score_object_only", action="store_true", default=True,
                        help="Dump object-prediction scores only, matching the paper setting.")
    parser.add_argument("--dump-both-directions", dest="dump_score_object_only", action="store_false",
                        help="Also dump subject-prediction scores using inverse-relation object queries.")
    parser.add_argument("--num-rels-for-hva", type=int, default=0,
                        help="Raw relation count used only if dumping inverse subject queries.")
    parser.add_argument("--hva-dump-split", type=str, default="test", choices=["valid", "test"],
                        help="When using test.py for score dumping, choose whether the test dataloader should point to validation or test graphs.")
