...
def inference():
    ...
    if not args.module == "Aggregator":
        ...
        # Preserve runtime HVA/score-dump CLI arguments before the checkpoint
        # config overwrites the main argument namespace.
        _hva_runtime_args = {
            "dump_score_dir": getattr(args, "dump_score_dir", None),
            "dump_score_object_only": getattr(args, "dump_score_object_only", True),
            "num_rels_for_hva": getattr(args, "num_rels_for_hva", 0),
            "hva_dump_split": getattr(args, "hva_dump_split", "test"),
        }
        args_json = json.load(open(config_path))
        args.__dict__.update(dict(args_json))
        for _k, _v in _hva_runtime_args.items():
            if _v is not None:
                setattr(args, _k, _v)
    ...
    # HVA score-dump support: reuse the TeMP test/inference path to export
    # validation scores when --hva-dump-split valid is requested.
    if getattr(args, "hva_dump_split", "test") == "valid":
        graph_dict_test = graph_dict_val
    module = {
              "Simple": SimplE,
              "Static": Static,
